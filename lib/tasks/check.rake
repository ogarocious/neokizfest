...
namespace :check do
  desc <<~DESC
    Record a Check by Mail payment and mark the refund request as Completed.

    Usage:
      bin/rails "check:record[RR-0151,220.00,Mary Grace Markham,YGYBC-K8S8W]"
      bin/rails "check:record[RR-0151,220.00,Mary Grace Markham,YGYBC-K8S8W,/path/to/screenshot.png]"

    Arguments (in order):
      1. confirmation_number   — e.g. RR-0151 (required)
      2. amount                — e.g. 220.00 (required)
      3. recipient_name        — e.g. "Mary Grace Markham" (required)
      4. check_confirmation    — bank confirmation number (required)
    ...
    Steps performed:
      ...
      3. Create Check by Mail Transfer record in Notion (Zelle Transfers DB)
  ...
  DESC
  task :record,
       [:confirmation_number, :amount, :recipient_name, :check_confirmation,
        :image_path] => :environment do |_t, args|

    puts "\n#{'=' * 70}"
    puts "  RECORD CHECK BY MAIL PAYMENT"
    ...
    # ── Step 3: Create Check Transfer record in Notion ─────────────────────
    print "  [3/4] Creating Check by Mail Transfer record in Notion... "

    transfer_result = zelle_service.create_transfer(
      recipient_name:         recipient,
      amount:                 amount,
      zelle_contact:          "Check by Mail",
      refund_request_page_id: request[:id],
      zelle_confirmation:     check_conf,
      proof_url:              proof_url,
      payment_method:         "Check",
      notes:                  "Check by Mail (bank bill pay). Conf: #{check_conf}. Est. delivery Apr 8. Recorded via rake task on #{Date.today}"
    )
    ...
    puts "\n  Done! #{confirmation} — #{recipient} — $#{'%.2f' % amount} — Completed (Check by Mail)"
    puts "  Check conf: #{check_conf} | Est. delivery: Apr 8"
